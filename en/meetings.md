# Team meetings

> ⚠️ **Work in progress.** Membership and frequency are still being discussed — until this is announced to the team, treat it as a working draft, not a schedule.

Slots are taken from the current 2026/27 timetable so that each meeting sits right next to its participants' lessons — nobody comes in early or waits afterwards. Each circle has a working language: the membership is built so the conversation runs without translation.

## Weekly

| Day | Time | Circle | Who | Language |
|---|---|---|---|---|
| Monday | 15:10–16:00 | Homeroom | Maria, Olga S., Elizaveta | RU |

## Every two weeks

| Day | Time | Circle | Who | Language |
|---|---|---|---|---|
| Friday | 16:05–17:00 | **Whole-team circle** | the whole primary team | RU |
| Tuesday | 12:05–12:55 | English | Maria, Albina, Sam | EN |
| Wednesday | 10:05–10:55 | Y4–Y5 pair | Margarita, Elizaveta | RU |
| Wednesday | 16:05–16:50 | Science | Albina, Olga S., Elizaveta | RU |
| Thursday | 10:05–10:55 | Y4 Science | Elizaveta, Sam | EN |
| Friday | 10:05–10:55 | Y2 pair | Margarita, Olga S. | RU |
| Friday | 14:10–15:00 | Academic circle | Olga S., Elizaveta, Gaiane, Olga P. | RU |

The whole-team circle and the academic circle fall in different weeks: in a whole-team week the academic circle does not meet.

## The whole-team circle

The only time in the week when the entire primary team can gather: Olga S., Elizaveta, Maria, Margarita, Gaiane, Olga P., Albina, Sam. Lessons end at 16:00 for everyone, and seven of the eight simply stay where they are.

This is the place for what does not fit into pairs and small circles: brainstorming, shared decisions, anything that concerns everyone at once. Every two weeks, fifty-five minutes.

## How the children close the day while the curator is in a circle

At 16:05 the curators normally hold reflection, so on whole-team and Science days the children's day closes differently — without losing the reflection itself:

- **Reflection happens at the end of prep** — led by the curator of that stage, as always: for the younger stage at the end of the 12:25–13:10 block, for the older one at the end of 13:20–14:05.
- **Home time, 16:05–16:30, is run by the assistants**: Pee Ann and Pee Mon with the younger stage, Pee Dee and Kru Pim with the older one.

The shape of the children's day does not change: reflection stays daily and stays with the curator — only its place inside the day moves.

## Once per module — on staff-training days

Subject verticals: **mathematics** (Olga P., Olga S., Sam) and **Russian language and literature** (Margarita, Gaiane). The unit of discussion is a skill and how it grows from Y2 to Y6: how far we have come, whether we all expect the same, and what the next teacher will receive.

Staff-training days: 12–13 October, 21–22 December, 15–17 February, 19–20 April.

Exception: the **language-and-literature vertical** (Margarita, Gaiane) meets **every three weeks, on Thursdays 9:05–9:55**. This is where the Y3 reading entry check lives. It is the pair's only shared window in the whole week — freed by moving three Y2 cells (28 Aug): Thai took the first lesson on Thursday and Russian moved to Friday 13:10. After it both teach back to back, so the meeting opens their day and holds nobody up.

## On signal, not on schedule

A small circle about one particular child: the curator plus the two or three subject teachers needed, within a week of the signal. The window is chosen to fit that group.

## How the circles work

There are two types, and they should not be mixed — otherwise the conversation always slides towards the children and the programme never gets discussed.

**Vertical** — one subject across the years. Led by the subject owner; horizon of a module and a year.

**Horizontal** — all the teachers around a particular group of children. Led by the curator; horizon of a week. The agenda is built **class by class**, not as one long block: a subject teacher comes for their own classes and leaves.

Separate from the circles is the curators' daily 8:15 briefing. That one is about today: who has arrived, what we are moving. Homeroom is about the child and the way we do things; unless these are kept apart, the circle turns into a repeat of the briefing within a month.

## What goes in writing instead of a meeting

The science topic table (Russian → English), the Peterson ↔ Cambridge mapping a module ahead, and the shared standard for presenting written work. Anything that passes on a fact rather than works out a decision.

*Checked against the timetable of 28 Aug (cells moved for the curators' lunch window and for the literacy window). Windows kept free of circles: the morning circle; 11:05–11:55, both curators' lunch; 11:55–14:05, lunch and prep with the children. The 16:05–16:30 window (reflection and home time) is open to circles only on the terms above: reflection at the end of prep, assistants afterwards. Three circles moved with the timetable: Homeroom from 14:10 to 15:10 on Monday, English from Tuesday 11:05 to 12:05, Y4 Science from Thursday 11:05 to 10:05. Re-check the slots whenever the master timetable is rebuilt.*
